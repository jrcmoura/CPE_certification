...
${excelFile}                ${EXECDIR}/DataTest/VariablesDataTest_R05.xlsx
...
Teste de certificacao - DUT_100-1001
    ...
    ${Register_uac_RXXXX}           Read Excel Cell     2       10      ${excelSheetName}
...
*** Test Cases ***
    ...
    # Tipo de Chamada   - SIPP para um numero previamente definido no arquivo Excel
    # Invoca a Keywords - Test_Case_SIPP_PSTN
Teste de certificacao - DUT_100-1002

    Open Excel Document     ${excelFile}        doc_id=doc1

    ${TestName}                     Read Excel Cell     3       2       ${excelSheetName}
    ${ShortStringPacketSniffer}     Read Excel Cell     3       6       ${excelSheetName}
    ${Register_uac_RXXXX}           Read Excel Cell     2       9       ${excelSheetName}
    ${uac_String_RXXXX}             Read Excel Cell     3       8       ${excelSheetName}
    ${SleepTest}                    Read Excel Cell     3       4       ${excelSheetName}
    ${SleepTest}                    Read Excel Cell     3       4       ${excelSheetName}
...
Teste de certificacao - DUT_100-1003
   ...
   # Abrir excel - Abrir excel, declara variavel e atribuir valor
    ...
    ${Register_uac_RXXXX}           Read Excel Cell     4       10      ${excelSheetName}
...
Test_Case_SIPP_PSTN
    ...
    IF  ${FlagTestSelected} <= 0
        ...
        Start Process       ${Register_uac_RXXXX}       shell=yes     alias=process_sipp_uas_reg
        Sleep       5 seconds
...
Test_Case_SIPP_SIPP
    ...
    IF  ${FlagTestSelected} <= 0
        ...
        Terminate Process       process_sipp_uas_reg        kill=true
        # Inicia processo - Register Ramal UAS RXXXX - Ver VariablesDataTest_R01.xlsx
        Start Process       ${Register_uac_RXXXX}       shell=yes     alias=process_sipp_uac_reg
        Sleep       5 seconds
        # Finaliza processo - process sipp uas register
        Terminate Process       process_sipp_uac_reg        kill=true
